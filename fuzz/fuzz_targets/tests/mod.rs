...
use crate::{assert_eq_events, assert_fields_eq, mock::*, pool_test};
use {
...
pub(crate) fn do_execute_delegation<P: PoolExt<Runtime>>(
    ...
    let request_before = pallet_pooled_staking::PendingOperations::<Runtime>::get(
        delegator,
    ...
    let request_after = pallet_pooled_staking::PendingOperations::<Runtime>::get(
        delegator,
        P::joining_operation_key(candidate, block_number),
    );
